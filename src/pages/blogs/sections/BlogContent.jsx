...
const BlogContentComponent = () => {
  ...
  return (
    <Grid container sx={{ marginTop: '0px', justifyContent: 'center', paddingTop: '70px' }}>
            <Grid sx={{paddingBottom: '100px', maxWidth: '1200px'}}>
                {BlogContent.map((course, index) => {
                    ...
                    return (
                        <Grid container sx={{ maxWidth: '1200px', gap: '0', flexDirection: isEven ? 'row' : 'row-reverse', flexWrap: 'wrap-reverse' }} key={index}>
                            <Grid item size={{ xs: 12, md: 6 }} sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '20px', ...(isSmallScreen && { paddingTop: '0px' }) }}>
                                ...
                                <p style={{ fontSize: '20px', ...(isSmallScreen && { marginTop: '0px' }), marginBottom: '0px' }}>{course.category}</p>
                                <p style={{ fontSize: '20px', marginTop: '8px', marginBottom: '20px' }}>{course.text}</p>

                                <Link to={course.path} style={{width: 'fit-content', borderRadius: '25px', backgroundColor: 'black', padding: '10px 20px', color: 'white', textDecoration: 'none', fontSize: '18px'}}>Read more</Link>
                            </Grid>
